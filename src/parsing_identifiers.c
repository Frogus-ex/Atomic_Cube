...
/*   Updated: 2026/01/07 12:06:59 by frogus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	grab_no_text(t_game *game, char *line, int i)
{
	...
	while (line[i] && game->nbr_text[0] == 0)
...
static void	grab_so_text(t_game *game, char *line, int i)
{
	...
	while (line[i] && game->nbr_text[1] == 0)
...
int	parse_identifer_line(t_game *game, char *av)
{
	...
	if (game->fd < 0)
		return (ft_error(game, "file doesn t exist"), 1);
	...
	if (count_text(game) == 0)
		return (ft_error(game, "nbr de textures invalide"), 0);
