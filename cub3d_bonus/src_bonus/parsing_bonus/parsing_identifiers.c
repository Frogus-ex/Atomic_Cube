...
/*   Updated: 2026/02/10 13:53:10 by tlorette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes_bonus/cub3D.h"

static int	grab_no_text(t_game *game, char *line, int i)
{
	int	y;

	y = 0;
	if (game->nbr_text[0] != 0)
		return (ft_error(NULL, "too many north textures"), 0);
...
static int	grab_so_text(t_game *game, char *line, int i)
{
	int	y;

	y = 0;
	if (game->nbr_text[1] != 0)
		return (ft_error(NULL, "too many south textures"), 0);
	while (line && line[i] == 32)
		i++;
...
int	check_id(t_game *game, char *line)
{
	...
	if (line[i] == 'N' && line[i + 1] == 'O')
		return (grab_no_text(game, line, i + 2));
	else if (line[i] == 'S' && line[i + 1] == 'O')
		return (grab_so_text(game, line, i + 2));
	else if (line[i] == 'W' && line[i + 1] == 'E')
		return (grab_we_text(game, line, i + 2));
	else if (line[i] == 'E' && line[i + 1] == 'A')
		return (grab_ea_text(game, line, i + 2));
	else if (line[i] == 'F' && line[i + 1] == 32)
		return (grab_f_text(game, line, i + 1));
	else if (line[i] == 'C' && line[i + 1] == 32)
		return (grab_c_text(game, line, i + 1));
