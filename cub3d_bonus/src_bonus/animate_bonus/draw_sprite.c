...
/*   Updated: 2026/02/06 13:12:06 by aautret          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_animate *animate)
{
	...
	while (relative_angle < -M_PI)
		relative_angle += 2 * M_PI;
	sprite->sprite_height = (int)((SCREEN_HEIGHT / sprite->distance) * TILE_SIZE
			* 0.5);
...
/**
 * @brief Dessine les animate sprite et fait le verification de distance
 *
 * - Check si le pixel est a l'ecran
 *
 * - Compare la distance du sprite avec le Z-buffer et si le sprite est
 * derriere le mur on n'affiche rien
 *
 * @param img
 * @param animate
 * @param sprite
		...
		t_sprite_proj *sprite, int pos[4])
{
	...
	screen_x = sprite->screen_x + pos[2];
	if (screen_x < 0 || screen_x >= SCREEN_WIDTH || pos[3] < 0)
		return ;
	if (sprite->distance >= img->z_buffer[screen_x])
		return ;
	tex_pos[0] = (pos[2] * animate->frame_width) / sprite->sprite_width;
	tex_pos[1] = (pos[0] * animate->frame_height) / sprite->sprite_height;
	color = get_sprite_pixel(animate->frame[animate->current_frame], tex_pos,
			animate);
	if (color != -1)
...
void	draw_sprite_3d(t_game *game, t_img *img, t_animate *animate)
{
	...
	pos[0] = 0;
	while (pos[0] < sprite.sprite_height && pos[1] + pos[0] < SCREEN_HEIGHT)
	{
